...
# Retrieve the gov1 key address
GOV1_ADDRESS=$(axelard keys show gov1 -a ${DEFAULT_KEYS_FLAGS})
if [ $? -ne 0 ]; then
  echo "Failed to retrieve gov1 key address. Please check your axelard configuration."
  exit 1
fi

# Generate the unsigned message
if ! axelard tx permission register-controller  "$GOV1_ADDRESS" --generate-only \
...
echo "Successfully setup permission controller"