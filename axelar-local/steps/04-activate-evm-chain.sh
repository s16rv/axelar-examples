...
# Set the chain variable, defaulting to 'ethereum' if not provided
CHAIN="${1:-ethereum}"
DIR="$(dirname "$0")"
...
if [ ! -x "$ACTIVATE_CHAIN_SCRIPT" ]; then
  echo "activate-chain.sh script not found or not executable at ${ACTIVATE_CHAIN_SCRIPT}."
  exit 1
fi

# Execute the activate-chain.sh script with the specified chain
if ! sh "$ACTIVATE_CHAIN_SCRIPT" "$CHAIN"; then
  echo "Failed to execute $ACTIVATE_CHAIN_SCRIPT with chain $CHAIN."
  exit 1
fi
